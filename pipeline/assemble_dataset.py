...
def fan_out_scores(scores_path, map_path):
    ...
    print(f"Merged:       {len(merged):,} (chunk-to-block mappings)")
...
def validate(bill_level, reference_path):
    ...
    if len(merged) == 0:
        print("  WARNING: Zero package_id matches with reference data.")
        return
    ...
    print(f"  Matched {len(merged):,} / {len(bill_level):,} bills with reference")
...
def main():
    ...
    if BILLS_119.exists():
        print("federal_content_bills_119.csv:")
        validate(bill_level, BILLS_119)
    if BILLS_EXT.exists():
        print("federal_content_bills_extended.csv:")
        validate(bill_level, BILLS_EXT)
